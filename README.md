...
データベースの一覧を表示し、「fleamarket」データベースがないことを確認してください。

<br>

### 演習2　（難易度：★☆☆）

「fleamarket」データベースがなければ作成してください。

<br>

### 演習3　（難易度：★☆☆）

「fleamarket」データベースに接続してください。
...
[SQLファイル](./sql/fleamarket.sql)

<br>

> 補足<br>ユーザマスタのパスワードに直接文字列を登録していますが、通常はセキュリティ強化のためハッシュ化などをして保存します。
...
フリマサイトにログインします。作成したアカウントのメールアドレスとパスワードでユーザマスタを検索し、アカウントが存在することを確認します。
